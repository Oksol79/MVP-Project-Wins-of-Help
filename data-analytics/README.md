...
The main KPIs and metrics were developed, and, together with the designer, key user flows and analytics events were agreed upon. Additionally, analytical hypotheses were formulated, and statistical methods for their verification were described.

The next stage was the development of a Python dataset for the project, taking into account the defined KPIs, metrics, and database structure, prepared by the front-end developer. The generated dataset consists of four tables: help_categories.csv, users.csv, help.csv, and events.csv.
...
The purpose of the Wings of Help web service is to facilitate effective collaboration between individuals or organizations in need of assistance and volunteers assisting. The document is intended for stakeholders, developers, designers, testers, analysts, and other team members involved in the project.
...
This document defines key user flows and the event model for the web service, supporting accurate product and event analytics. It helps measure the effectiveness of key user scenarios and ensures consistency between UX, backend, and data analytics. The document also prepares the system for implementing event tracking.
...
This document describes statistical methods used to analyze product analytics for the “Wings of Help” web service. These methods support accurate KPI analysis and the evaluation of A/B test results, enabling data-driven product decisions. The document also helps ensure the correct use of statistical tests and a consistent analytical approach across teams.
...
The Wings of Help web service enables interaction between people or organizations in need of help and volunteers, allowing users to create, publish, view, and manage help requests. The platform allows requests to be created, processed by volunteers, and content to be moderated by administrators.  
The dataset was created to simulate the platform, test the functionality of the web service, analyze user-volunteer interactions, and support further research on optimizing help request processing and scaling the system. Currently, the web platform is being developed as an MVP with basic functionality for a quick launch and future scalability.
...
The Wings of Help web service enables interaction between people or organizations in need of help and volunteers through a centralized platform for managing help requests. The project dataset was created in Python and follows the Postgres SQL database structure, key performance indicators (KPIs), and metrics defined in the project documentation.
...
The web platform of “Wings of Help” demonstrates stable user growth and a gradual increase in user involvement: Engagement Rate has increased to 45%, and WAU is stable, without sharp declines. This indicates the formation of an active user base and the growth in value of this web service.   
...
The main need at the moment is to scale up the offers and improve the matching processes, since a significant share of requests remains in the new status. The web platform has demonstrated strong demand and positive growth dynamics, so the next stage is to strengthen the volunteer base and increase the conversion of interactions to more effectively close requests for help.  
...
This dashboard is designed for a comprehensive analysis of the effectiveness of the Wings of Help web platform. It allows you to assess the balance between demand and supply of assistance, the level of coverage of requests by volunteers, the speed of response to requests, and the overall operational effectiveness of the case processing process. The use of key KPIs and analytical visualizations enables you to identify bottlenecks in the platform's operations, pinpoint risk areas, and formulate management recommendations to improve the quality of the web service. The developed Wings of Help: Platform Health & Demand-Supply Analysis dashboard is shown in Fig. 2. 
...
As a result of building the dashboard, it was found that the Wings of Help platform operates in a state of stable imbalance between demand and supply. In particular, the number of requests for help consistently exceeds the number of offers of help by more than 2 times, leading to unmet demand.  
The analysis of process efficiency revealed that only a portion of the created requests are processed, with a small percentage reaching completion, indicating a bottleneck in fulfilling demand by volunteers. At the same time, the average and median time to take a request into work are about 34.5 hours, indicating a stable, but not instantaneous, response time. The presence of cases with delays of up to 72 hours indicates an uneven workload, low volunteer involvement, or limited resources.    
Therefore, the key insights are the structural supply shortage, the need to increase volunteer involvement, and the potential to optimize the response process to reduce user waiting times.  
